ChargebeeRails.configure do |config|
  ...
  config.chargebee_site = Rails.application.credentials.chargebee[Rails.env.to_sym][:site]
  config.chargebee_api_key = Rails.application.credentials.chargebee[Rails.env.to_sym][:api_key]
  ...
  config.webhook_api_path = 'webhooks/chargebee_rails_event'
